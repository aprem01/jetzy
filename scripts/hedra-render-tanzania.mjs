/**
 * Pre-renders Aria-talking videos for the Tanzania safari + Zanzibar tour.
 * Reads from /demo-audio-tz/, writes to /demo-aria-tz/.
 *
 * Run:  HEDRA_API_KEY=xxx node scripts/hedra-render-tanzania.mjs
 */
...
const AUDIO_DIR = path.join(PUBLIC, 'demo-audio-tz');
const OUT_DIR = path.join(PUBLIC, 'demo-aria-tz');
...
async function run() {
  ...
  console.log(`Found ${ariaLines.length} Tanzania Aria lines`);
  ...
  for (let i = 0; i < todo.length; i++) {
    ...
    outManifest.push({ id: line.id, speaker: line.speaker, text: line.text, scene: line.scene, duration: line.duration, audioFile: line.file, videoFile: `/demo-aria-tz/${outFile}` });
