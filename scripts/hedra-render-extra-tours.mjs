/**
 * Renders Aria-talking videos for the 3 new tours sequentially:
 *   nepal → iceland → marrakech
 * Reads from /demo-audio-{np|is|ma}/, writes to /demo-aria-{np|is|ma}/.
 */
...
const TOURS = [
  { audio: 'demo-audio-np', out: 'demo-aria-np' },
  { audio: 'demo-audio-is', out: 'demo-aria-is' },
  { audio: 'demo-audio-ma', out: 'demo-aria-ma' },
];
...
async function runOne(tour) {
  const AUDIO_DIR = path.join(PUBLIC, tour.audio);
  const OUT_DIR = path.join(PUBLIC, tour.out);
  ...
  console.log(`Found ${ariaLines.length} Aria lines`);
  ...
  console.log('→ Uploading Aria image');
  ...
  for (let i = 0; i < todo.length; i++) {
    ...
    outManifest.push({ id: line.id, speaker: line.speaker, text: line.text, scene: line.scene, duration: line.duration, audioFile: line.file, videoFile: `/${tour.out}/${outFile}` });
    await writeFile(existingManifestPath, JSON.stringify(outManifest, null, 2));
  }
  console.log(`✓ ${tour.out} done`);
}

async function run() {
  for (const t of TOURS) await runOne(t);
  console.log('\n━━━ ALL 3 TOURS DONE ━━━');
}
