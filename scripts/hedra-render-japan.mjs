/**
 * Pre-renders Aria-talking videos for the Japan tour via Hedra Character-3.
 * Same pipeline as hedra-render-demo.mjs but reads from /demo-audio-jp/
 * and writes to /demo-aria-jp/.
 *
 * Run:  HEDRA_API_KEY=xxx node scripts/hedra-render-japan.mjs
 */
...
const AUDIO_DIR = path.join(PUBLIC, 'demo-audio-jp');
const OUT_DIR = path.join(PUBLIC, 'demo-aria-jp');
...
async function run() {
  ...
  console.log(`Found ${ariaLines.length} Japan Aria lines (Marco lines skipped)`);
  ...
  for (let i = 0; i < todo.length; i++) {
    ...
    outManifest.push({
      id: line.id, speaker: line.speaker, text: line.text, scene: line.scene,
      duration: line.duration, audioFile: line.file,
      videoFile: `/demo-aria-jp/${outFile}`,
    });
    await writeFile(existingManifestPath, JSON.stringify(outManifest, null, 2));
  }

  console.log('\n✓ Japan Hedra batch done');
}
